...
async def get_referral_status(referred_id: int) -> str | None:
    async with get_conn() as db:
        cur = await db.execute("SELECT status FROM referrals WHERE referred_id=?", (referred_id,))
        row = await cur.fetchone()
        return row[0] if row else None


async def pending_referrals_for_reminder(hours: int, max_count: int) -> list[dict]:
    """Referrals still 'pending' whose referred user hasn't joined the
    required channels/groups yet, due for a reminder: at least `hours`
    since signup (or since the last reminder), and under `max_count`
    reminders already sent."""
    async with get_conn() as db:
        db.row_factory = None
        cur = await db.execute(
            "SELECT referred_id, referrer_id, reminder_count FROM referrals "
            "WHERE status='pending' AND reminder_count < ? "
            "AND datetime(COALESCE(last_reminder_at, created_at)) <= datetime('now', ?)",
            (max_count, f"-{hours} hours"),
        )
        rows = await cur.fetchall()
        cols = [d[0] for d in cur.description]
        return [dict(zip(cols, r)) for r in rows]
...
async def bump_referral_reminder(referred_id: int) -> None:
    async with get_conn() as db:
        await db.execute(
            "UPDATE referrals SET reminder_count = reminder_count + 1, "
            "last_reminder_at = datetime('now') WHERE referred_id=?",
            (referred_id,),
        )
        await db.commit()
...
async def register_shared_link_view(token: str) -> dict | None:
    async with get_conn() as db:
        cur = await db.execute(
            ...
            "is_active=CASE WHEN one_time=1 THEN 0 ELSE is_active END "
...
async def delete_shared_link(token: str, owner_id: int | None = None) -> bool:
    """Permanently remove a link. If owner_id is given, only that owner's
    link can be deleted (used from the user-facing /mylinks flow); pass
    None for admin force-delete."""
    async with get_conn() as db:
        if owner_id is None:
            cur = await db.execute("DELETE FROM shared_links WHERE token=?", (token,))
        else:
            cur = await db.execute(
...
async def link_admin_stats() -> dict:
    async with get_conn() as db:
        cur = await db.execute("SELECT COUNT(*), COALESCE(SUM(views),0) FROM shared_links")
        total, total_views = await cur.fetchone()
        cur = await db.execute("SELECT COUNT(*) FROM shared_links WHERE is_active=1")
        (active,) = await cur.fetchone()
        return {"total": total or 0, "active": active or 0, "total_views": total_views or 0}


async def recent_shared_links(limit: int = 20) -> list[dict]:
    async with get_conn() as db:
        cur = await db.execute(
            "SELECT sl.*, u.username FROM shared_links sl LEFT JOIN users u ON u.user_id=sl.owner_id "
...
async def log_admin_action(admin_id: int, action: str, details: str = "") -> None:
    """Generic audit-log write. Call this from any admin-only handler that
    changes state (bans, payouts, deletions, config toggles, broadcasts)
    so there's a record of who did what."""
    async with get_conn() as db:
        await db.execute(
            "INSERT INTO admin_logs (admin_id, action, details) VALUES (?,?,?)",
...
async def recent_admin_logs(limit: int = 30) -> list[dict]:
    async with get_conn() as db:
        cur = await db.execute(
...
async def list_force_join_channels(active_only: bool = True, required_only: bool = False) -> list[dict]:
    async with get_conn() as db:
        query = "SELECT * FROM force_join_channels WHERE 1=1"
        params: list = []
        if active_only:
            query += " AND is_active=1"
        if required_only:
            query += " AND is_required=1"
        query += " ORDER BY is_required DESC, title"
...
async def add_force_join_channel(
    ...
    chat_type: str = "channel", is_required: bool = True,
) -> None:
    if chat_type not in {"channel", "group"}:
        raise ValueError("Unsupported chat_type")
    async with get_conn() as db:
        await db.execute(
            "INSERT INTO force_join_channels (channel_id, title, invite_link, chat_type, is_required) "
            "VALUES (?,?,?,?,?) ON CONFLICT(channel_id) DO UPDATE SET "
            "title=excluded.title, invite_link=excluded.invite_link, "
            "chat_type=excluded.chat_type, is_required=excluded.is_required, is_active=1",
            (channel_id, title, invite_link, chat_type, int(is_required)),
        )
        await db.commit()
...
async def toggle_force_join_required(channel_id: int) -> None:
    async with get_conn() as db:
        await db.execute(
            "UPDATE force_join_channels SET is_required = 1 - is_required WHERE channel_id=?",
            (channel_id,),
        )
        await db.commit()
...
# ------------------------------------------------------- referral engine ---
async def recent_referrals_count(referrer_id: int, minutes: int) -> int:
    """How many NEW referred users this referrer picked up in the last N
    minutes — used to rate-limit / flag suspicious referral farming."""
    async with get_conn() as db:
        cur = await db.execute(
            "SELECT COUNT(*) FROM referrals WHERE referrer_id=? "
            "AND created_at > datetime('now', ?)",
            (referrer_id, f"-{int(minutes)} minutes"),
        )
...
async def top_referrers(limit: int = 10) -> list[dict]:
    async with get_conn() as db:
        cur = await db.execute(
            "SELECT r.referrer_id, u.username, COUNT(*) as active_count "
            "FROM referrals r LEFT JOIN users u ON u.user_id = r.referrer_id "
            "WHERE r.status='active' GROUP BY r.referrer_id "
            "ORDER BY active_count DESC LIMIT ?",
            (limit,),
        )
        rows = await cur.fetchall()
        cols = [d[0] for d in cur.description]
...
async def active_referral_count(referrer_id: int) -> int:
    async with get_conn() as db:
        cur = await db.execute(
...
async def all_active_referred_ids() -> list[int]:
    """Referred-user IDs currently counted 'active' — used by the periodic
    revalidation job to detect users who left after being verified."""
    async with get_conn() as db:
        cur = await db.execute("SELECT referred_id FROM referrals WHERE status='active'")
        return [row[0] for row in await cur.fetchall()]


async def create_milestone(threshold: int, reward_coin: int) -> int:
    async with get_conn() as db:
        cur = await db.execute(
            "INSERT INTO referral_milestones (threshold, reward_coin) VALUES (?,?)",
            (threshold, reward_coin),
        )
        await db.commit()
        return cur.lastrowid


async def list_milestones(active_only: bool = True) -> list[dict]:
    async with get_conn() as db:
        query = "SELECT * FROM referral_milestones"
        if active_only:
...
async def delete_milestone(milestone_id: int) -> None:
    async with get_conn() as db:
        await db.execute(
            "UPDATE referral_milestones SET is_active=0 WHERE milestone_id=?", (milestone_id,)
        )
        await db.commit()


async def claimed_milestone_ids(user_id: int) -> set[int]:
    async with get_conn() as db:
        cur = await db.execute(
            "SELECT milestone_id FROM referral_milestone_claims WHERE user_id=?", (user_id,)
        )
        return {row[0] for row in await cur.fetchall()}


async def claim_milestone(user_id: int, milestone_id: int) -> bool:
    async with get_conn() as db:
        try:
            await db.execute(
                "INSERT INTO referral_milestone_claims (user_id, milestone_id) VALUES (?,?)",
