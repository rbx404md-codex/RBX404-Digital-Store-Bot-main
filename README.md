...
- 📌 Force Join V2 — একাধিক Channel **ও** Group, প্রতিটি আলাদাভাবে Required/Optional, admin
  চাইলে পুরো Gate সাময়িকভাবে অফ রাখতে পারবে
- 🧠 Referral Engine — referral only counts as **active** after the referred user joins every
  required Channel/Group (`pending` → `active`), periodic leave-detection (background sweep,
  `REFERRAL_REVALIDATION_MINUTES`), suspicious-activity flag (rate-based), milestone bonus
  rewards, admin Top Referrers view, configurable "revoke reward if referred user leaves later"
- 🔁 Multi-level referral — referrer's own referrer gets a Level-2 bonus (`REFERRAL_LEVEL2_REWARD_COIN`,
  admin on/off toggle) when a level-1 referral makes their first purchase
- ⏰ Join reminder sweep — nudges referred users stuck in `pending` after `REFERRAL_REMINDER_HOURS`
  hours, up to `REFERRAL_REMINDER_MAX_COUNT` times, and notifies the referrer as soon as their
  referral actually verifies
- 🏆 Referral leaderboard — top active referrers, from the Referral menu or the admin panel
- 🔗 LinkGen V2 — per-link detail view (views/expiry countdown/last accessed), permanent delete,
  admin Link Management overview (stats + force-delete), automatic expired-link cleanup (every 30 min)
- 🌐 Mini web portal V2 — OG metadata for link previews, Share button, branded 404 page
- 🐳 Docker support — `Dockerfile`, `docker-compose.yml`, healthcheck, non-root user, `railway.toml`
- 🌍 Multi-language infrastructure — `locales/strings.py` + `utils/i18n.py`; বাংলা ও English পূর্ণাঙ্গ,
  Hindi/Urdu/Russian/Arabic নির্বাচনযোগ্য (আপাতত English fallback দিয়ে) — Settings → Language থেকে বদলানো যায়
- 🛡️ Security hardening — per-user flood-control middleware, generic admin audit log
  (`log_admin_action`) wired into Force Join/Link/Coupon changes, admin "🧾 Audit Log" viewer
